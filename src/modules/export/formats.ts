...
export function buildCsvExport(snapshot: ExportSnapshot) {
  ...
  const headers = [
    ...
    'pressure_group_before_interval',
    'pressure_group_after_interval',
    'pressure_group_end',
    'residual_nitrogen_seconds',
    'water_type_code',
    'entry_type_code',
    'visibility',
    'current',
  ...
  const rows = data.dives
    ...
    .map((dive) => {
      ...
      return [
        ...
        csvCell(dive.safetyStop),
        csvCell(dive.safetyStopSeconds),
        csvCell(dive.surfaceIntervalSeconds),
        csvCell(dive.pressureGroupBeforeInterval, true),
        csvCell(dive.pressureGroupAfterInterval, true),
        csvCell(dive.pressureGroupEnd, true),
        csvCell(dive.residualNitrogenSeconds),
