...
export const getDiveOutput = z.looseObject({
  ...
  safetyStopSeconds: z.number().int().nullable(),
  pressureGroupBeforeInterval: optionalText.describe('Dive-table pressure group letter'),
  pressureGroupAfterInterval: optionalText.describe('Dive-table pressure group letter'),
  pressureGroupEnd: optionalText.describe('Dive-table pressure group letter'),
  residualNitrogenSeconds: z.number().int().nullable(),
  captureSource: z.enum(['manual', 'computer']),
  ...
  tanks: z.array(
    z.looseObject({
      ...
      gasRole: optionalText.describe(
        'bottom, deco, or travel when the computer planned it',
      ),
  ...
  events: z.array(
    z.looseObject({
      ...
      kind: z.enum(['gas_switch', 'alert', 'marker']),
      code: z.string(),
      label: z.string(),
  ...
  sources: z.array(
    z.looseObject({
      ...
      role: z
        .string()
        .describe('"produced" created the dive; "matched" attached to an existing one'),
      device: z.looseObject({
        name: optionalText,
        serialNumber: optionalText,
        softwareVersion: optionalText,
      }),
      contributions: z.looseObject({
        profileSamples: z.number().int(),
        tanks: z.number().int(),
        events: z.number().int(),
        buddies: z.number().int(),
  ...
  profile: z.object({
    ...
    samples: z.array(
      z.looseObject({
        ...
        ndlSeconds: z.number().int().nullable(),
        timeToSurfaceSeconds: z.number().int().nullable(),
        cnsPercent: z.number().int().nullable(),
        nitrogenLoadPercent: z.number().int().nullable(),
      }),
    ),
  }),
})
