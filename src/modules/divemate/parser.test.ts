...
describe('parseDiveMateDatabase', () => {
  ...
  test('ignores discarded status 2 dives and their profiles', async () => {
    ...
    database.exec(`
      ...
      CREATE TABLE Tank (ID INTEGER, LogID INTEGER);
      CREATE TABLE Pictures (ID INTEGER, LogID INTEGER, Path TEXT);
      ...
      INSERT INTO Tank VALUES (1, 1), (2, 3);
      INSERT INTO Pictures VALUES (1, 1, 'discarded.jpg'), (2, 3, 'kept.jpg');
    `)
    database.close()
    ...
    expect(snapshot.discardedDiveExternalIds).toEqual(['1'])
    expect(snapshot.profileSamples.map((sample) => sample.diveExternalId)).toEqual(['3'])
    expect(snapshot.tanks.map((tank) => tank.diveExternalId)).toEqual(['3'])
    expect(snapshot.pictures.map((picture) => picture.diveExternalId)).toEqual(['3'])
  })
})
