...
type DiveMateDiveStatus = 'manual' | 'computer' | 'discarded'

function diveStatus(value: unknown): DiveMateDiveStatus {
...
function mapDive(row: SourceRow): DiveMateDive | null {
  ...
  const status = diveStatus(row.Status)
  if (!source || !diveDate || status === 'discarded') return null
  return {
    ...source,
...
function mapProfileSamples(row: SourceRow): DiveMateProfileSample[] {
  if (diveStatus(row.Status) === 'discarded') return []
...
export async function parseDiveMateDatabase(
  ...
  try {
    ...
    const logbookRows = readRows(database, 'Logbook')
    const discardedDiveExternalIds = compact(
      logbookRows.map((row) =>
        diveStatus(row.Status) === 'discarded' ? externalId(row.ID) : null,
      ),
    )
    const discardedDiveIds = new Set(discardedDiveExternalIds)
    return {
      ...
      dives: compact(logbookRows.map(mapDive)),
      tanks: compact(readRows(database, 'Tank').map(mapTank)).filter(
        (tank) => !discardedDiveIds.has(tank.diveExternalId),
      ),
      pictures: compact(readRows(database, 'Pictures').map(mapPicture)).filter(
        (picture) =>
          !picture.diveExternalId || !discardedDiveIds.has(picture.diveExternalId),
      ),
      profileSamples: logbookRows.flatMap(mapProfileSamples),
    }
  } finally {
    database.close()
  }
